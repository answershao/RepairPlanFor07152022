function [temp_variables, conflict_acts_info] = waitfor_othertime_schedule(project_para, data_set, iter_variables, conflict_acts_info, timeoff, forward_set, need_global_activity, cpm, time)
    ...
    lft = find_lft(project_para, data_set, cpm, iter_variables); %最晚结束时间

    %5.3.2  根据cur_need_global_activity确定冲突活动顺序列表
    %cur_conflict(按照项目权重、活动工期、全局需求量三个优先规则)
    if ~isempty(cur_need_global_activity)
        [cur_conflict] = find_cur_conflict_repair(data_set, iter_variables, cur_need_global_activity, lft);
