function [allocated_acts_information] = all_act_infos(schedule_solution)
    ...
    for time = 1:length(allocated_acts)
        ...
        ind = ~cellfun(@isempty, each_act); %找到非空cell数组
        [~, index3] = find(ind ~= 0); %index3为非空cell所在的位置
        iter_each_act = each_act(index3);

        for conflict_number = 1:length(iter_each_act) %从第一个活动开始调取基本信息
            each_act_info = iter_each_act{conflict_number}; %获得该活动的所有信息：{技能值，资源序号，项目活动[i, j]，结束时间 temp_variables.local_end_times，开始时间 temp_variables.local_start_times}
