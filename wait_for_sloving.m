function [temp_schedule_solution,objective] = wait_for_sloving(project_para, data_set, constant_variables, iter_variables, timeoff, variables_with_time,conflict_acts_info)
% project_para
L = project_para.L;
...
%     [~, index_return] = ismember(time, return_time);
%
%     if index_return ~= 0 %说明该time是员工返回时刻
%         timeoff.return_time = return_time(index_return);
%         timeoff.leave_staff = leave_staff(index_return); %员工的请假和返回，配对出现，即哪个员工走对应矩阵上的哪个员工便在一定时间返回
%
%         % 遍历找返回时刻的活动信息
%
%         for i = 1:length(allocated_acts_information)
%             eachtime_act_info = allocated_acts_information{i};
%
%             if eachtime_act_info.performing_time == timeoff.return_time %找到了返回时刻的活动信息
%                 %员工复职
%                 eachtime_act_info.Lgs(:, timeoff.leave_staff) = data_set.Lgs(1:end, timeoff.leave_staff);
%                 eachtime_act_info.skill_num(1, :) = (sum(eachtime_act_info.Lgs ~= 0, 2))';
%                 allocated_acts_information{i} = eachtime_act_info;
%             end
%
%         end
%
...
% objective_staff = abs(conflict_act_info. .resource_worktime - timeoff.resource_worktime); %修复目标值f2：资源工作时间之和偏差，找iter_variables.resource_worktime？
% objective = (1/2) * objective_act + (1/2) * objective_staff;
