function [temp_variables, conflict_acts_info] = waitfor_othertime_allocate_resource(data_set, iter_variables, timeoff, cur_conflict, time)
    %data_set
    ...
    for v = 1:length(cur_conflict) % 閬嶅巻鍐茬獊鍒楄〃涓?姣忎釜鏁扮??
        ...
        if temp_skill_num(skill_cate(pro, act)) >= GlobalSourceRequest(pro, act) %闲置资源满足，可分配，则当前时刻的allocated_set无需更新
            ...
            %
            if pro == timeoff.leave_activity_infos.pro && act == timeoff.leave_activity_infos.act %请假员工离开，需要旧人+新人的技能值之和
                unalready_workload = timeoff.leave_activity_infos.unalready_workload;
            else
                unalready_workload = GlobalSourceRequest(pro, act) * data_set.d(act, 1, pro);
            end
