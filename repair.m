...
average_objective = sum(sum(save_cycle_objective)) / (files * project_para.cycles); %仿真均值
average_cpu = toc / (files * project_para.cycles);
average_leave_duration = sum(sum(save_cycle_leave_duration)) / (files * project_para.cycles); %仿真均值

saved_infos = zeros(3, file + 1);
saved_infos(1, 1:file) = save_cycle_objective;
saved_infos(1, file + 1) = average_objective;
